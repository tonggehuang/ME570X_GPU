...
void wbTimer_stop(wbTimerKind_t kind, string msg, const char *file,
  ...
  if (wbLogger_printOnLog && node) {
	...
	int millisec = floor(wbTimerNode_getElapsedTime(node)*1e-6);
	int microsec = floor(wbTimerNode_getElapsedTime(node)*1e-3) - millisec * 1000;
	int nanosec = wbTimerNode_getElapsedTime(node) - microsec * 1000 - millisec * 1e6;
	std::cout << msg << " " << wbTimerNode_getElapsedTime(node) << "ns" << std::endl;
	std::cout << "Formatted: " << millisec << "ms " << microsec << "us " << nanosec << "ns" << std::endl;
  }
